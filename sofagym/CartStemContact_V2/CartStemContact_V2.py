...
class CartStem_V2():
    ...
    def onEnd(self, rootNode, collisionGroup=1):
        ...
        stemVisu.addObject('OglModel', name="model", src="@loader", scale3d=[0.01, 0.25, 0.01], color=[1, 0, 0, 1], updateNormals=False, translation = [self.init_pos[0], -2.5, 0], rotation = [-90, 0, 0])
        stemVisu.addObject('SkinningMapping')

        #Add stem collision
        stemCollis = self.cartStem.addChild('stemCollis')
        stemCollis.addObject('MeshSTLLoader', name='loader', filename=pathMesh+"obstacleColli.stl", scale3d=[0.01, 0.25, 0.01], translation = [self.init_pos[0], -2.5, 0], rotation = [-90, 0, 0])
    ...
    def _create_beam(self, rootNode):
        ...
        pos = [[i*(tot_length/nb_frame), init_pos[1], init_pos[2] ] for i in range(nb_frame+1)]
        edges = [[i, i+1] for i in range(nb_frame)]

        model.addObject('EdgeSetTopologyContainer', position=pos, edges=edges)
        model.addObject('MechanicalObject', template='Rigid3',
                            position=[p + [0, 0, 0, 1] for p in pos],
                            showObject=False, drawMode=0, showObjectScale=0.5, rotation = [0, -90, 0], translation = [init_pos[0], 0, 0])
