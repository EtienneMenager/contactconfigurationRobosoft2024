...
from CartStemContact_V2 import CartStem_V2, Contacts
from Controller import ControllerCartStem
from CartStemContact_V2Toolbox import rewardShaper, goalSetter, sceneModerator, applyAction
...
def createScene(rootNode, config = {"source": [0, -50, 10],
    ...
    header(rootNode, alarmDistance=1.0, contactDistance=0.1, tolerance = 1e-6, maxIterations=100, gravity = [0,0,-981.0], dt = config['dt'], mu = "0")
    ...
    max_v = 2
    beam_config = {'init_pos': [init_x, 0, 0], 'tot_length': 30, 'nbFrames': 20}
    cartstem_config = {"init_pos": [init_x, 0, 0], "cart_size": [2, 2, 5], "max_move": max_move,  "max_v": max_v, "dt": config["dt"],  "beam_config":beam_config, "use_direct_action":True}
    contact_config = {"init_pos": [0, 0, 12], "cube_size": [2, 1, 2], "cube_x": config["cube_x"]}

    cartstem = CartStem_V2(cartstem_config = cartstem_config)
