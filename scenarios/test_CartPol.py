...
parser.add_argument("-pl", "--pre_learning", help="Use pre-learning",
                     action="store_true")
...
assert n_state in [2, 4]
if not USEV:
    PATH_DATA = "./Data/CartPol/n_state="+str(n_state)
    PATH_SAVE = './Results/CartPol/n_state='+str(n_state)
else:
    PATH_DATA = "./Data/CartPol/v_n_state="+str(n_state)
    PATH_SAVE = './Results/CartPol/v_n_state='+str(n_state)
...
from lms import LMS
from dataCartPol import CartPolDatasetState,CartPolDatasetTransition, get_data_loader
import random
import numpy as np
import json
...
def evaluate_agent(env, lms, n_starts=1, render = False, target = None):
    reward_sum = 0
    liste_reward = []
    ...
    for _ in range(n_starts):
        ...
        while (not done):
            ...
            action = node.get_action(observation, goal_id)
...
nodes = [[] for _ in range(n_state)]
for i in range(n_state):
    nodes[i] += list(range(n_state))
if USEV and n_state == 4:
    nodes[0] = [0, 1, 2]
    nodes[3] = [1, 2, 3]
    print("CONNECTIVITY:", nodes)
p = [0 for _ in range(n_state)]
config = {"d_input": 4, "d_hidden": 64, "d_output": 64,
        "action_dim": 2, "action_lim": [0, 1], "continous": False,
        "nodes": nodes, "init_active": int(n_state-1)/2,
        "p": p, "batchsize": args.batchsize, "gamma": [args.gamma, args.gamma, args.gamma],
        "reward_external": args.reward_external, "use_LSTM_encoder": False, "pre_learning": args.pre_learning,
...
if TRAIN_STATES:
    #Load data
    dataset_train = CartPolDatasetState(path = PATH_DATA, validation = False, n_state = n_state)
    dataset_validation = CartPolDatasetState(path = PATH_DATA, validation = True, n_state = n_state)
...
if TRAIN_ACTION:
     with torch.autograd.set_detect_anomaly(True):
        ...
        for e in range(1, n_epoch):
            ...
            reward = func_reward(reward, observation, target = TARGET)
            observation = torch.tensor(observation).type(torch.float)
            ...
            if e > n_random_actions:
                if next_id!= id and inOption and (deterministic or args.pre_learning):
            ...
            if done:
                if inOption and e > n_random_actions and (deterministic or args.pre_learning):
                    for i in range(len_option):
                        sOpt, rOpt = stateOption[i], rewardOption[i]
                        node.add_evaluator_transition(sOpt, local_action_evaluator, rOpt, nextstate, real_done, node, goal_id, id, next_id, penality = PENALITY)
            ...
            if e%freq_val==0:
                current_reward, current_list_reward = evaluate_agent(env_test, lms, n_starts=10, render = False, target= TARGET)
            ...
            # if e%2000==0:
            #     current_reward, _ = evaluate_agent(env, lms, n_starts=1, render = True, target= TARGET)
